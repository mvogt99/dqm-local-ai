/**
 * RootCauseAnalysis Component - V89 Enhancement
...
function RootCauseAnalysis({ apiBase }) {
  ...
  // V88: Filter failed results by DAMA dimension
  const filteredFailedResults = useMemo(() => {
    if (selectedDimensions.length === 0) return failedResults;
    return failedResults.filter(r => selectedDimensions.includes(r.dama_dimension));
  }, [failedResults, selectedDimensions]);
  ...
  return (
    <div className="root-cause-analysis">
      <h2>Root Cause Analysis <span style={{ fontSize: '12px', background: '#FF5722', color: 'white', padding: '2px 8px', borderRadius: '4px', marginLeft: '8px' }}>{VERSION}</span></h2>
      ...
      <div className="failed-results">
        <div className="section-header" style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
          <h3>Failed DQ Results ({filteredFailedResults.length})</h3>
